...
int main()
{
	...
	while (window.isOpen()) // opens game window
	{
		...
		if (p.box.getPosition().x > map.mapWidth) p.box.setPosition(map.mapWidth, p.box.getPosition().y);
		if (p.box.getPosition().y < 0) p.box.setPosition(p.box.getPosition().x, 0);
		if (p.box.getPosition().y > map.mapHeight) p.box.setPosition(p.box.getPosition().x, map.mapHeight);
		...
		{
			bool spw = true;
			Vector2f pos = Vector2f(rand() % (map.mapWidth-25), rand() % (map.mapHeight-25));
